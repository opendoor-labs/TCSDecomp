...
tcs_decomp_estim = function (y, freq = NULL, decomp = NULL, trend_spec = NULL, det_obs = F, 
  ...
  `%fun%` = foreach::`%dopar%`
  fit = foreach::foreach(i = iter, .combine = "comb", .packages = c("data.table", "Matrix", "maxLik", "imputeTS", "Rcpp"), .export = c("SSmodel"), 
                         .noexport = c("kalman_filter", "kalman_smoother")) %fun% {
